...
def test_non_dict_artifact_coerced_to_empty_dict(bad):
    ...
    assert out["total"] is None and out["dual_order_share"] is None
...
def test_bool_and_non_finite_not_numeric():
    assert _is_number(0.625) and _is_number(3)
...
def test_slice_summary_happy_path():
    # agree+disagree+tie = 5 dual of total 8 -> 0.625
    out = _slice_summary(_stats(agree=3, disagree=1, tie=1, single=2, offline=1))
    assert out == {"total": 8, "dual_order_tasks": 5, "dual_order_share": 0.625}


def test_slice_summary_zero_total_reports_zero_counts():
    # coherent all-zero counts: no share is defined, but total/dual are reported as 0.
    out = _slice_summary(_stats())
    assert out == {"total": 0, "dual_order_tasks": 0, "dual_order_share": None}
...
def test_slice_summary_missing_key_withholds():
    ...
    assert out == {"total": None, "dual_order_tasks": None, "dual_order_share": None}


@pytest.mark.parametrize("stats", [
    {"agree": -1, "disagree": 0, "tie": 0, "single": 0, "offline": 0},   # negative
    {"agree": 1.0, "disagree": 0, "tie": 0, "single": 0, "offline": 0},  # float
    {"agree": True, "disagree": 0, "tie": 0, "single": 0, "offline": 0},  # bool
])
...
def test_single_and_multi_kinds():
    single = summarize_dual_order_share(_stats(agree=3, disagree=1, tie=1, single=2, offline=1))
    assert single["kind"] == "single"
    assert single["total"] == 8 and single["dual_order_tasks"] == 5
    assert single["dual_order_share"] == 0.625 and single["partitions"] is None

    multi_art = {"per_repo": [], **_stats(agree=1, disagree=1, tie=0, single=0, offline=0)}
    multi = summarize_dual_order_share(multi_art)
    assert multi["kind"] == "multi" and multi["partitions"] is None
    assert multi["total"] == 2 and multi["dual_order_share"] == 1.0
...
def test_summary_always_includes_required_keys():
    for art in ({}, _stats(agree=1, disagree=0, tie=0, single=0, offline=0),
                {"per_repo": [], **_stats()},
                {"generalization_gap": 0.1, "tuned": _stats(agree=1, disagree=0, tie=0),
                 "held_out": _stats(agree=1, disagree=0, tie=0)}):
        assert _REQUIRED_KEYS <= set(summarize_dual_order_share(art))


def test_generalization_partitions_and_overall():
    art = {
        "generalization_gap": 0.1,
        "tuned": _stats(agree=2, disagree=1, tie=0, single=1, offline=0),   # total 4, dual 3
        "held_out": _stats(agree=1, disagree=0, tie=0, single=1, offline=0),  # total 2, dual 1
    }
    out = summarize_dual_order_share(art)
    assert out["kind"] == "generalization"
    assert out["total"] == 6 and out["dual_order_tasks"] == 4       # summed across partitions
    assert out["dual_order_share"] == round(4 / 6, 3)               # 0.667
    assert out["partitions"]["tuned"]["dual_order_share"] == 0.75
    assert out["partitions"]["held_out"]["dual_order_share"] == 0.5
...
def test_generalization_partial_partition_withholds_overall():
    ...
    art = {
        "generalization_gap": 0.1,
        "tuned": _stats(agree=2, disagree=1, tie=0, single=1, offline=0),
        "held_out": {"judge_order_stats": {"agree": 1}},
    }
    out = summarize_dual_order_share(art)
    assert out["total"] is None and out["dual_order_tasks"] is None
...
def test_headline_with_counts_exact_format():
    summary = summarize_dual_order_share(_stats(agree=3, disagree=1, tie=1, single=2, offline=1))
    assert dual_order_share_headline(summary) == "dual-order share: 62.5% (5/8 categorized task(s))"
...
def test_headline_none_share_shows_na():
    ...
    summary = {"total": 5, "dual_order_tasks": 3, "dual_order_share": None}
    assert dual_order_share_headline(summary) == "dual-order share: n/a (3/5 categorized task(s))"


def test_headline_nan_share_shows_na():
    summary = {"total": 5, "dual_order_tasks": 3, "dual_order_share": float("nan")}
    assert dual_order_share_headline(summary) == "dual-order share: n/a (3/5 categorized task(s))"


def test_headline_non_dict_summary_coerced():
    assert dual_order_share_headline("not a dict") == "dual-order share: no judge stats available"
...
def test_summarize_does_not_mutate_artifact():
    art = {
        "generalization_gap": 0.1,
        "tuned": _stats(agree=2, disagree=1, tie=0, single=1, offline=0),
        "held_out": _stats(agree=1, disagree=0, tie=0, single=1, offline=0),
    }
