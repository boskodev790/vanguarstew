...
def load_repo_set(path) -> RepoSet:
    ...
    try:
        ...
        # binary file passed by mistake) makes json.load raise UnicodeDecodeError while decoding
        # the text stream. It is a ValueError subclass — neither json.JSONDecodeError nor an
        # OSError — so it slips past the other branches; wrap it like every other load failure.
        raise RepoSetError(f"repo-set config is not valid UTF-8 ({path}): {exc}") from exc
